...
function seed(overrides: Seed = {}): void {
  ...
  ).run(1, overrides.heading ?? 'Items', 2, overrides.sectionContent ?? 'section body', 0);
  db.prepare('INSERT INTO chunks (id, document_id, content) VALUES (?,?,?)').run('c1', 1, 'chunk');
  db.close();
}
...
describe('lintCorpus', () => {
  ...
  it('fails on zero-width characters in a heading', () => {
    seed({ heading: `Items${ZWSP}` });
    expect(failedNames()).toContain('no zero-width chars in sections.heading');
  });

  it('fails on a body-less section', () => {
    seed({ sectionContent: '   ' });
    expect(failedNames()).toContain('no body-less sections');
  });
  ...
  it('fails on the phantom 21.9', () => {
    seed({ minecraftVersion: '21.9' });
    expect(failedNames()).toContain('minecraft_version holds only Minecraft versions');
  });

  it('accepts a genuinely unversioned document', () => {
    // 359 reference pages have no Minecraft version. Null is honest; guessing
    // would not be.
