...
pnpm exec vitest run >/tmp/p4_test.log 2>&1 && pass "vitest green"             || fail "test"
...
# CLEANROOM_MCP_SKIP_AUTO_UPDATE keeps boot offline — otherwise startup fetches docs.db (~743 MB).
CLEANROOM_MCP_SKIP_AUTO_UPDATE=1 timeout 10 bash -c "$SERVER_CMD </dev/null >/tmp/p4_boot.log 2>&1"; rc=$?
...
grep -q 'Auto-update skipped' /tmp/p4_boot.log && pass "boot skipped DB auto-update (no network fetch)" || fail "boot did not honor CLEANROOM_MCP_SKIP_AUTO_UPDATE"
